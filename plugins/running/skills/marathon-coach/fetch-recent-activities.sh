#!/usr/bin/env bash
# fetch-recent-activities.sh — download recent FIT files and run fit-analyzer for the marathon-coach skill
#
...
# The Garmin fetch script lives in the analyze-activity skill directory
...
[[ -n "$USER_ARG" ]] || die "Usage: fetch-recent-activities.sh <user> [count]"
...
# ---------------------------------------------------------------------------
# Detect source: Garmin Connect or Runalyze
# ---------------------------------------------------------------------------
GARMIN_EMAIL=$(read_yaml_field "$CONFIG" garmin_email)
if [[ -n "$GARMIN_EMAIL" ]]; then
    command -v python3 >/dev/null 2>&1 || die "python3 not found (required for Garmin Connect)"
    [[ -f "$GARMIN_FETCH" ]] || die "fetch-fit-garmin.py not found at $GARMIN_FETCH"
    exec python3 "$GARMIN_FETCH" "$USER_ARG" --batch "$COUNT"
fi
...
# ---------------------------------------------------------------------------
# Resolve N most recent activities (all sports, skip placeholder future dates)
# ---------------------------------------------------------------------------
...
[[ -n "$ACTIVITIES" ]] || die "No recent activities found in Runalyze"
